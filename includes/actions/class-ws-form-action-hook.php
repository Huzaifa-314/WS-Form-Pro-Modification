<?php
	...
	class WS_Form_Action_Hook extends WS_Form_Action {
		...
		public function load_config($config) {
			...
			if($this->priority == '') { $this->priority = 0; } else { $this->priority = absint($this->priority); }
		}
		...
		public function config_meta_keys($meta_keys = array(), $form_id = 0) {
			...
			$config_meta_keys = array(
				...
				'action_' . $this->id . '_priority'	=> array(
					...
					'default'	=>	190,
					'help'		=>	__('Should the WordPress hook be run before or after other actions?', 'ws-form')
				)
			);
