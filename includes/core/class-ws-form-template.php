			...
			foreach($config_files as $config_file) {
				...
				foreach($config_object->template_categories as $template_category_index => $template_category) {
					...
					usort($template_category->templates, function($a, $b) {
						...
						return ($a_label === $b_label) ? 0 : (($a_label > $b_label) ? 1 : 0);
					});
...
<div class="wsf-template" data-id="blank">
	<svg class="wsf-responsive" viewBox="0 0 <?php esc_attr_e($svg_width); ?> <?php esc_attr_e($svg_height); ?>"><rect height="100%" width="100%" fill="<?php esc_attr_e($color_form_background); ?>"/><text fill="<?php esc_attr_e($color_default) ?>" class="wsf-template-title"><tspan x="<?php echo is_rtl() ? esc_attr($svg_width - 5) : 5; ?>" y="16"><?php esc_html_e('Blank', 'ws-form'); ?></tspan></text></svg>
...
<div class="wsf-template" title="<?php esc_html_e($template->label); ?>">
<?php
					// Echo SVG
...
?>
	<a class="wsf-button wsf-button-primary wsf-button-full" href="<?php esc_attr_e(WS_Form_Common::get_plugin_website_url('', 'add_form')); ?>" target="_blank"><?php esc_html_e('Upgrade to PRO', 'ws-form'); ?></a>
<?php
					} else {
?>
	<button class="wsf-button wsf-button-primary wsf-button-full" data-action="wsf-add-<?php esc_attr_e($action_id); ?>" data-id="<?php esc_attr_e($template->id); ?>"><?php esc_html_e('Use Template', 'ws-form'); ?></button>
<?php
					}

					if($template->preview_url !== false) {
?>
	<a class="wsf-preview" href="<?php esc_attr_e($template->preview_url); ?>" target="_blank"><?php WS_Form_Common::render_icon_16_svg('visible'); ?> <?php esc_html_e('Preview Template', 'ws-form'); ?></a>
<?php
					}
?>
	</div>
