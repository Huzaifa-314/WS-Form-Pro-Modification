<?php
	...
	class WS_Form_Form_Stat extends WS_Form_Core {
		...
		public function db_get_time_bounds() {
			...
			$date_time_local_start = strtotime(get_gmt_from_date(date('Y-m-d 00:00:00', WS_Form_Common::wp_version_at_least('5.3') ? current_datetime()->getTimestamp() : current_time('timestamp', 0))));
		...
		public function db_get_chart_data_time($time_from_gmt = false, $time_to_gmt = false) {
			...
			$sql = sprintf("SELECT MIN(date_added) AS date_added_from, MAX(date_added) AS date_added_to FROM %s%s ORDER BY date_added;", $this->table_name, $where_sql, $this->form_id);
		...
		public function add_view_php_valid() {
			...
			$wp_load_paths = apply_filters('wsf_wp_load_paths', array(
			...
			if(!$add_view_php_error && is_admin() && function_exists('curl_version')) {
				...
				$curl_options = array(
					...
					CURLOPT_USERAGENT		=> 'WSForm/' . WS_FORM_VERSION . ' (wsform.com)',
