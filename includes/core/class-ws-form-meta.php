<?php
	...
	class WS_Form_Meta extends WS_Form_Core {
		...
		public function db_read($meta_key) {
			...
			$sql = sprintf("SELECT meta_value FROM %s WHERE parent_id = %u AND meta_key = '%s' LIMIT 1", self::db_get_table_name(), $this->parent_id, esc_sql($meta_key));
		...
		public function db_update_from_array($meta_data_array, $lookups = false, $bypass_user_capability_check = false) {
			...
			foreach($meta_data_array as $key => $value) {
				...
				$sql = sprintf("SELECT id FROM %s WHERE parent_id = %u AND meta_key = '%s' LIMIT 1", self::db_get_table_name(), $this->parent_id, esc_sql($key));
				$id = $wpdb->get_var($sql);
				if($id) {
