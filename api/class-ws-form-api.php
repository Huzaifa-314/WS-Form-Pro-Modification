<?php
	...
	class WS_Form_API {
		...
		public function api_json_response($data = false, $form_id = 0, $history = false, $form_full = true, $form_published = false, $form_parse = false, $form_public = false) {
			...
			// Output JSON return
			echo apply_filters('wsf_api_response_json', $response_json); 
		...
		public function api_rest_api_init() {
			...
			register_rest_route(WS_FORM_RESTFUL_NAMESPACE, '/form/(?P<form_id>[\d]+)/stat/reset/', array('methods' => 'POST', 'callback' => array($plugin_api_form_stat, 'api_reset'), 'permission_callback' => function () { return true; }));
