<?php
	...
	class WS_Form_API_Field extends WS_Form_API {
		...
		public function api_dropzonejs($parameters) {
			...
			if(is_array($_FILES[$field_name]['error'])) {
				...
				foreach($_FILES[$field_name]['error'] as $file_index => $file_error) {
					...
					$_FILES[$file_id] = array(
						...
						'name'  => $_FILES[$field_name]['name'][$file_index],
						'type'  => $_FILES[$field_name]['type'][$file_index],
						'tmp_name' => $_FILES[$field_name]['tmp_name'][$file_index],
						'error' => $file_error,
						'size'  => $_FILES[$field_name]['size'][$file_index]
					);

					try {

						WS_Form_File_Handler::dropzonejs_upload($file_id, $attachment_ids, $upload_path, $file_size, $file_min_size, $file_max_size);
				...
				try {
					...
					WS_Form_File_Handler::dropzonejs_upload($field_name, $attachment_ids, $upload_path, $file_size, $file_min_size, $file_max_size);
			...
			return array('error' => false, 'attachment_ids' => $attachment_ids);
		}
		// API - Select - AJAX
