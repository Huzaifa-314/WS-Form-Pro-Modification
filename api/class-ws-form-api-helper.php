<?php
	...
	class WS_Form_API_Helper extends WS_Form_API {
		...
		public function api_framework_detect($parameters) {
			...
			$wp_remote_get_response = wp_remote_get($url, $wp_remote_get_args);
			...
			foreach($links as $link) {
				...
				if($exclude_filenames !== false) {
					...
					foreach($exclude_filenames as $exclude_filename) {
						...
						if(strpos($css_url, $exclude_filename) !== false) { $exclude = true; break; }
					}
				}
				if($include_filenames !== false) {
					...
					foreach($include_filenames as $include_filename) {
						...
						if(strpos($css_url, $include_filename) !== false) { $exclude = false; break; }
					}
				}
				if($exclude) { continue; }

				// Request CSS document
				$wp_remote_get_response = wp_remote_get($css_url, $wp_remote_get_args);
		...
		public function api_support_contact_submit() {
			...
			// Check for error
			if($api_response_error = is_wp_error($wp_remote_get_response)) {

				// Handle error
				$api_response_error_message = $wp_remote_get_response->get_error_message();
				...
				$api_response_headers = wp_remote_retrieve_headers($wp_remote_get_response);
				$api_response_body = wp_remote_retrieve_body($wp_remote_get_response);
				$api_response_http_code = wp_remote_retrieve_response_code($wp_remote_get_response);
			}
