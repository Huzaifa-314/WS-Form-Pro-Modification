(function($) {
			...
			for(var section_icon_index in section_icons) {
				...
				switch(section_icons_style) {
					...
					default :
						...
						var section_icon_html = (section_icon_svg !== false) ? ('<svg class="wsf-section-icon" focusable="false" height="' + section_icons_size + '" width="' + section_icons_size + '" viewBox="0 0 16 16" style="display: block; height: auto; max-width: 100%;">' + section_icon_svg + '</svg>') : section_icon_text;
				}

				section_icons_array.push('<a href="#" data-action="wsf-section-' + section_icon_type + '-icon" class="wsf-section-' + section_icon_type + '" style="cursor: ' + section_icon_cursor + '; margin: 0; -webkit-margin-end: ' + skin_spacing_small + 'px; margin-inline-end: ' + skin_spacing_small + 'px; padding: 0;" title="' + ws_this.html_encode(section_icon_label) + '" aria-label="' + ws_this.html_encode(section_icon_label) + '" data-color-on="' + section_icons_color_on + '" data-color-off="' + section_icons_color_off + '" data-repeatable-section-id="' + repeatable_section_id + '">' + section_icon_html + '</a>');
			}

			$(this).html('<div class="wsf-section-icons" style="display: flex; justify-content: ' + section_icons_horizontal_align + '; list-style-type: none; margin: 0; padding: 0; user-select: none;">' + section_icons_array.join('') + '</div>');
		});
	}
	...
	$.WS_Form.prototype.section_repeatable_navigation_events = function(section_id) {
		...
		$('[data-action="wsf-section-move-up-button"][data-repeatable-section-id="' + section_id + '"], [data-action="wsf-section-move-up-icon"][data-repeatable-section-id="' + section_id + '"]', this.form_canvas_obj).not('[data-init-repeatable-section]').on('click', function(e) {
			...
			// Calculations
			if(typeof(this.form_calc) === 'function') { ws_this.form_calc(false, section_id); }
	...
	$.WS_Form.prototype.section_action_off = function(obj, id) {
		...
		obj.addClass('wsf-section-' + id + '-disabled').filter(':button').attr('disabled', '').attr('aria-disabled', 'true');
	...
	$.WS_Form.prototype.section_action_on = function(obj, id) {
		...
		obj.removeClass('wsf-section-' + id + '-disabled').filter(':button').removeAttr('disabled').removeAttr('aria-disabled');
