	...
	$.WS_Form.prototype.form_password_strength_meter_process = function(obj) {
		...
		if(password_strength_invalid) {
			...
			if(field_value_strength < parseInt(password_strength_invalid, 10)) {
				...
				var password_strength_invalid_label = this.get_object_meta_value(field, 'text_password_strength_invalid', '');
				if(password_strength_invalid_label == '') { password_strength_invalid_label = this.language('password_strength_invalid'); }
				this.set_invalid_feedback(obj, password_strength_invalid_label);
