(function($) {
	...
	$.WS_Form.prototype.form_conversational_settings = function() {
		...
		this.section_obj_first = $('.wsf-section:visible', this.form_obj).first();

		// Get last visible section object
		this.section_obj_last = $('.wsf-section:visible', this.form_obj).last();

		// Get all sections that should be full height
		this.sections_full_height = $('.wsf-form-conversational-section-full-height', this.form_obj);
	...
	$.WS_Form.prototype.form_conversational_navigation = function() {
		...
		this.form_obj.append(navigation_html);

		// Get navigation element
		var navigation_obj = $('.wsf-form-conversational-nav', this.form_obj);
	...
	$.WS_Form.prototype.form_conversational_focus = function() {
		...
		// Field focussed
		this.form_obj.on('focus', '.wsf-field, .wsf-button, [tabindex="0"]', function(event, method) {
	...
	$.WS_Form.prototype.form_conversational_focus_section = function(scroll_to, focus_first_field) {
		...
		// Hide color pickers
		$('[data-type="color"] input', this.form_obj).each(function() {
		...
		$('[data-type="datetime"] input', this.form_obj).each(function() {
	...
	$.WS_Form.prototype.form_conversational_key_down = function() {
		...
		$('.wsf-field, .wsf-button', this.form_obj).on('keydown', function(e) {
	...
	$.WS_Form.prototype.form_conversational_scroll = function() {
		...
		$(window).on('scroll', function(event) {
			...
			$('fieldset', ws_this.form_obj).each(function() {
	...
	$.WS_Form.prototype.form_conversational_section = function() {
		...
		// Section clicked
		$('.wsf-section', this.form_obj).on('click', function(e) {
		...
		// Repeatable sections
		this.form_obj.on('click', '[data-action="wsf-section-add-button"], [data-action="wsf-section-add-icon"], [data-action="wsf-section-delete-button"], [data-action="wsf-section-delete-icon"]', function() {
	...
	$.WS_Form.prototype.form_conversational_field = function() {
		...
		// Field changed
		$('.wsf-field-wrapper:not([data-type="signature"],[data-type="rating"],[data-type="color"],[data-type="checkbox"],[data-type="price_checkbox"],[data-type="radio"],[data-type="price_radio"],[data-type="range"],[data-type="price_range"]) .wsf-field', this.form_obj).on('change', function(event) {

			ws_this.form_conversational_field_changed($(this));
		});

		// Prevent select option clicking causing a section change
		$('.wsf-field-wrapper select', this.form_obj).on('click', function(event) {

			event.stopPropagation();
		});

		// Field changed - Rating
		$('[data-type="rating"] li', this.form_obj).on('click', function(event) {
		...
		// Field changed - Radio
		$('.wsf-field[type="radio"]', this.form_obj).on('click', function(event) {
	...
	$.WS_Form.prototype.form_conversational_field_changed = function(field_obj) {
		...
		if(field_wrapper[0] === field_wrapper_last[0]) {
			...
			if(field_wrapper_next.length) {
				...
				var field_next_obj = $('.wsf-field', field_wrapper_next);
