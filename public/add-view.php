<?php
	...
	if(
		...
		!isset($_SERVER['SCRIPT_FILENAME']) ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		!isset($_SERVER['HTTP_REFERER']) ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		!isset($_SERVER['HTTP_HOST']) ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		!isset($_SERVER['REQUEST_METHOD']) ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		(strtolower($_SERVER['REQUEST_METHOD']) !== 'post') ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		!isset($_POST) ||	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		!isset($_POST['wsffid'])	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	) {
		exit;
	}

	// Check referrer
	if(strtolower(parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST)) != strtolower($_SERVER['HTTP_HOST'])) { exit; }	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

	// Read form ID
	$form_id =  absint($_POST['wsffid']);
	if($form_id === 0) { exit; }
