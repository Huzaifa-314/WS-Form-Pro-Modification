...
?>
<a class="wsf-button wsf-button-small wsf-button-information" href="<?php esc_attr_e(admin_url('admin.php?page=ws-form-edit&id=' . $form_id)); ?>"><?php WS_Form_Common::render_icon_16_svg('edit'); ?> <?php esc_html_e('Edit', 'ws-form'); ?></a>
<?php
		}
?>
<a class="wsf-button wsf-button-small" href="<?php esc_attr_e(WS_Form_Common::get_preview_url($form_id)); ?>" target="_blank"><?php WS_Form_Common::render_icon_16_svg('visible'); ?> <?php esc_html_e('Preview', 'ws-form'); ?></a>
<?php
...
<form method="get">
<input type="hidden" name="_wpnonce" value="<?php esc_attr_e(wp_create_nonce('wp_rest')); ?>">
<input type="hidden" name="orderby" value="<?php esc_attr_e($order_by_query_var); ?>">
<input type="hidden" name="order" value="<?php esc_attr_e($order_query_var); ?>">
...
<!-- Submissions Actions -->
<form action="<?php esc_attr_e(WS_Form_Common::get_admin_url()); ?>" id="ws-form-action-do" method="post">
<input type="hidden" name="_wpnonce" value="<?php esc_attr_e(wp_create_nonce('wp_rest')); ?>">
...
<input type="hidden" id="ws-form-id" name="id" value="<?php esc_attr_e($form_id); ?>">
<input type="hidden" id="ws-form-submit-id" name="submit_id" value="">
<input type="hidden" id="ws-form-date-from" name="date_from" value="<?php esc_attr_e(WS_Form_Common::get_query_var_nonce('date_from', '', false, false, true, 'POST')); ?>">
<input type="hidden" id="ws-form-date-to" name="date_to" value="<?php esc_attr_e(WS_Form_Common::get_query_var_nonce('date_to', '', false, false, true, 'POST')); ?>">
<input type="hidden" id="ws-form-paged" name="paged" value="<?php esc_attr_e(WS_Form_Common::get_query_var_nonce('paged', '', false, false, true, 'POST')); ?>">
<input type="hidden" id="ws-form-status" name="ws-form-status" value="<?php esc_attr_e(WS_Form_Common::get_query_var_nonce('ws-form-status', '', false, false, true, 'POST')); ?>">
</form>
<!-- /Submissions Actions -->
...
<script>
	...
	(function($) {
		...
		$(function() {
			...
			wsf_obj.wp_list_table_submit(<?php esc_html_e($form_id); ?>);
		});
...
<div id="wsf-form-submit-export-popup" class="wsf-form-popup-progress">
	...
	<div class="wsf-form-popup-progress-inner"><img src="<?php esc_attr_e(WS_FORM_PLUGIN_DIR_URL) . 'admin/images/loader.gif'; ?>" class="wsf-responsive" width="256" height="256" alt="<?php esc_html_e('Your export is being created...', 'ws-form'); ?>" /><p><?php esc_html_e('Your export is being created...', 'ws-form'); ?></p>
...
?>	wsf_form_json_config.<?php esc_html_e($key); ?> = <?php echo wp_json_encode($config); ?>;
<?php
	}
