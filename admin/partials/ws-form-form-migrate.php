...
?><option value="<?php esc_attr_e($migration['id']); ?>"<?php
...
?>><?php esc_html_e($migration['label'] . ' (' . $migration['version'] . ')'); ?><?php
...
?><option value="<?php esc_attr_e($form['id']); ?>"<?php if($wsf_form_migrate['form_id_source'] == $form['id']) { ?> selected<?php } ?>><?php esc_html_e($form['label']) . ' (' . esc_html__('ID', 'ws-form') . ': ' . esc_html($form['id']) . ')'; ?></option>
<?php
...
<option value="submissions"<?php if($wsf_form_migrate['method'] == 'submissions') { ?> selected<?php } ?><?php if($record_count == 0) { ?> disabled<?php } ?>><?php esc_html_e('Submissions', 'ws-form'); ?> (<?php esc_html_e(sprintf(_n('%u record', '%u records', $record_count, 'ws-form'), $record_count)); ?>)</option>
</select></td>
</tr>
<?php
	}
...
?><option value="<?php esc_attr_e($form['id']); ?>"<?php if($form['id'] == $wsf_form_migrate['form_id_destination']) { ?> selected<?php } ?>><?php esc_html_e($form['label']) . ' (' . esc_html__('ID', 'ws-form') . ': ' . esc_html($form['id']) . ')'; ?></option>
<?php
			}
?>
</select></td>
...
<?php
			...
			echo '<p>';
			esc_html_e(sprintf(__('You first need to migrate or create a form in %s to migrate submission data into.', 'ws-form'), WS_FORM_NAME_GENERIC));
			echo '</p>';
		}
	}
	...
	(function($) {
		...
		$(function() {
			...
			function wsf_import_button() {
				...
				var button_enable = (<?php esc_html_e($show_field_mapping ? 'true' : 'false'); ?> && field_mapping_valid) || <?php esc_html_e(($wsf_form_migrate['method'] == 'form') ? 'true' : 'false'); ?>;
...
<?php
	foreach($settings as $setting) {
?>
					'wsf_form_migrate[<?php esc_html_e($setting); ?>]' : $('[name="wsf_form_migrate[<?php esc_html_e($setting); ?>]"]').val(),
<?php
	}
?>
					'wsf_field_mapping[source][]' : $('[name="wsf_field_mapping[source][]"]').map(function() {return $(this).val();}).get()
...
?>
				...
				wsf_obj.api_call('migrate/submission/import', 'POST', form_data, function(response) {
					...
					if(remaining) {
						...
						success_message_array.push('<a href="admin.php?page=ws-form-submit&id=<?php esc_attr_e($wsf_form_migrate['form_id_destination']); ?>" target="_blank">' + $.WS_Form.this.language('migrate_view_submissions') + '</a>');
						var success_message = success_message_array.join(' ');
...
?><option value="<?php esc_attr_e($field_id); ?>"<?php if(($field_id_source !== false) && ($field_id_source == $field_id)) { ?> selected<?php } ?>><?php esc_html_e($field_data['label']); ?> (ID: <?php esc_html_e($field_id); ?>)</option>
<?php
		}
?>
</select>
...
?><option value="<?php esc_attr_e($field->id); ?>"<?php if(($field_id_destination !== false) && ($field_id_destination == $field->id)) { ?> selected<?php } ?>><?php esc_html_e($field->label); ?> (<?php esc_html_e($field->id); ?>)</option>
<?php
		}
?>
</select>
