	...
	class WS_Form_Admin {
		...
		public function enqueue_scripts($hook) {
			...
			$ws_form_settings = array(
				...
				// Date / time
				'date'							=> WS_Form_Common::wp_version_at_least('5.3') ? current_datetime()->format(get_option('date_format')) : date(get_option('date_format'), current_time('timestamp')),
				'time'							=> WS_Form_Common::wp_version_at_least('5.3') ? current_datetime()->format(get_option('time_format')) : date(get_option('time_format'), current_time('timestamp')),
...
<script>
	...
	(function($) {
		...
		$(function() {
			...
			// Modal open
			$('[data-slug="ws-form"] .deactivate a, [data-slug="ws-form-pro"] .deactivate a').on('click', function(e) {
			...
			$('[data-action="wsf-feedback-submit"]').on('click', function() {
				...
				$.ajax({
					...
					url: '<?php esc_html_e(WS_Form_Common::get_api_path('helper/deactivate-feedback-submit/')); ?>',
					data: {

						'wsf_nonce_field_name' : '<?php esc_attr_e(WS_FORM_POST_NONCE_FIELD_NAME); ?>',
						'wsf_nonce': '<?php esc_attr_e(wp_create_nonce(WS_FORM_POST_NONCE_ACTION_NAME)); ?>',
					...
					beforeSend: function(xhr) {
						...
						xhr.setRequestHeader('X-WP-Nonce', '<?php esc_html_e(wp_create_nonce('wp_rest')); ?>');
					},
...
?><a href="#" class="button wsf-button-add-form"><span class="wsf-button-add-form-icon"><?php
	...
	echo WS_Form_Common::get_admin_icon('#888888', false);

?></span><?php esc_html_e(sprintf(
			...
			$('[data-action]', $('#wsf-add-form-modal')).on('click', function() {
				...
				switch(action) {
					...
					case 'wsf-inject' :
						...
						// Build shortcode
						var shortcode = '[<?php esc_html_e(WS_FORM_SHORTCODE); ?> id="' + id + '"]';
					...
					case 'wsf-add' :
						...
						location.href = '<?php echo esc_url(WS_Form_Common::get_admin_url('ws-form-add')); ?>';
...
?><option value="<?php esc_attr_e($form['id']); ?>"><?php esc_html_e($form['label']); ?> (ID: <?php esc_html_e($form['id']); ?>)</option>
<?php
		}
?>
</select>
...
<p><a href="<?php esc_attr_e(WS_Form_Common::get_admin_url('ws-form-add')); ?>"><?php esc_html_e('Click here to create a form', 'ws-form'); ?></a></p>
<?php
	}
?>
</form>
...
?><option value="<?php esc_attr_e($form['id']); ?>"<?php if($form['id'] == $wsf_dashboard_widget_form_id) { ?> selected<?php } ?>><?php esc_html_e($form['label']); ?> (<?php esc_html_e('ID', 'ws-form'); ?>: <?php esc_html_e($form['id']); ?>)</option>
<?php
				}
?>
</select>
...
?><option value="<?php esc_attr_e($date_range_key); ?>"<?php if($date_range_key == $wsf_dashboard_widget_date_range) { ?> selected<?php } ?>><?php esc_html_e($date_range['label']); ?></option>
<?php
				}
?>
</select>
	...
	(function($) {
		...
		function wsf_dashboard_widget_render() {
			...
			$.ajax({
				...
				method: 'GET',
				url: '<?php esc_attr_e(WS_Form_Common::get_api_path('form/stat/get-chart-data/')); ?>',
				beforeSend: function(xhr) {

					xhr.setRequestHeader('X-WP-Nonce', '<?php esc_attr_e(wp_create_nonce('wp_rest')); ?>');
				},
				data: {
					...
					'random'		: '<?php esc_attr_e(wp_generate_password(12, false, false)); ?>'
				},
				statusCode: {
...
<h2 class="wsf-align-center"><?php
	...
	/* translators: %s = Presentable name (e.g. WS Form PRO) */
	esc_html_e(sprintf(__('Welcome to %s', 'ws-form'), WS_FORM_NAME_PRESENTABLE));

?></h2>
<p class="wsf-align-center"><?php esc_html_e('This dashboard widget will show statistics for any submissions you receive.'); ?></p>
<div class="wsf-align-center"><a class="wsf-button wsf-button-small wsf-button-information" href="<?php esc_attr_e(WS_Form_Common::get_admin_url('ws-form-add')); ?>" title="<?php esc_attr_e('Add New', 'ws-form'); ?>"><?php
